...
<?php include 'alert-file.php'; ?>
  ...
  <div class="product-header">
    ...
    <?php }else{ ?>
    <button style="visibility: hidden;">Add Product</button>
    <?php } ?>
  </div>
...
foreach($results as $row)
{
    ...
    echo "<td>".number_format(floatval($row['price'])*floatval($row['quantity']),2)."</td>";
    echo "<td>";
...
<div class="modal" id="edit-product-modal">
    <div class="modal-content">
      ...
      <form action="product.php" method="post" enctype="multipart/form-data">
        <input type="hidden" id="product-idE" name="product-id" placeholder="id">
        <input type="text" id="product-nameE" name="product-name" placeholder="name" required>
        <input type="number" id="product-quantityE" name="product-quantity" placeholder="quantity" required>
        <select id="product-categoryE" name="product-category" placeholder="category" required>
          <option value="">Select Category</option>
        ...
        <textarea id="product-descriptionE" name="product-description" placeholder="description"></textarea>
        <input type="text" id="product-priceE" name="product-price" placeholder="price" required>
        
        <!-- Add the input file element for selecting an image -->
        <label for="product-image">Product Image:</label>
        <input type="file" id="product-imageE" name="product-image" value="">
        <!-- <button type="submit" name="edit">edit2</button> -->
        <button type="submit" name="edit" class="add-btn" id="e-btn">Edit Product</button>
        <button type="button" id="cancel-add" class="cancel-btn">Cancel</button>
      </form>
    </div>
  </div>
...
var modalE = document.getElementById("edit-product-modal");
...
editButtons.forEach((button)=>{
   ...
   button.addEventListener('click', (event)=>{
       ...
       document.getElementById("product-idE").value = id;
       document.getElementById("product-imageE").value = picture;
       document.getElementById("product-nameE").value = name;
       document.getElementById("product-descriptionE").value = description;
       document.getElementById("product-priceE").value = price;
       document.getElementById("product-quantityE").value = quantity;
       
       // Select the corresponding option in the category dropdown
       const categoryDropDown = document.getElementById("product-categoryE");
